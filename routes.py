...
async def login_for_access_token(db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    # Proceed with token creation if user exists
...
async def generate_response(user_input: UserInput, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Generate response
    new_user_input_ids = tokenizer.encode(user_input.text + tokenizer.eos_token, return_tensors='pt')
    bot_input_ids = new_user_input_ids
    chat_history_ids = model.generate(bot_input_ids, max_length=1000, pad_token_id=tokenizer.eos_token_id)
    response = tokenizer.decode(chat_history_ids[:, bot_input_ids.shape[-1]:][0], skip_special_tokens=True)
    
    # Save to database
    user_message = ChatContent(chat_id=chat.chat_id, message_content=user_input.text, message_type=0)
    bot_response = ChatContent(chat_id=chat.chat_id, message_content=response, message_type=1)
    db.add(user_message)
    db.add(bot_response)
    db.commit()
...
def get_user_messages(user_id: int, db: Session = Depends(get_db)):
    user_messages = db.query(ChatContent).join(Chat).filter(Chat.user_id == user_id).all()
